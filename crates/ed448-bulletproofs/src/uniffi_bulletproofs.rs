...
pub fn generate_input_commitments(values: Vec<Vec<u8>>, blinding: Vec<u8>) -> Vec<u8> {
    if values.iter().any(|v| v.len() != 56) || blinding.len() % 56 != 0 {
        println!("invalid scalar size");
        return vec![];
    }
    let values_scalars: Vec<Scalar> = values
        .iter()
        .map(|v| Scalar::from_bits(v.as_slice().try_into().unwrap()))
        .collect();
    let blinding_scalars: Vec<_> = (0..(blinding.len() / 56))
        .map(|i| Scalar::from_canonical_bytes(blinding[i * 56..(i + 1) * 56].try_into().unwrap()))
        .collect();
    ...
    let commits: Vec<Point> = values_scalars
        .iter()
        .zip(new_blinds)
        .map(|(v, b)| pc_gens.commit(*v, b))
        .collect();
    return commits
        .iter()
...
pub fn generate_range_proof(
    ...
    let existing_blinds: Vec<_> = (0..(blinding.len() / 56))
        .map(|i| Scalar::from_canonical_bytes(blinding[i * 56..(i + 1) * 56].try_into().unwrap()))
        .collect();
    let mut total = Scalar::zero();
    ...
    let blinding_bytes = blinding_scalars
        .iter()
...
pub fn verify_range_proof(proof: Vec<u8>, commitment: Vec<u8>, bit_size: u64) -> bool {
    if commitment.len() % 56 != 0 {
        return false;
    }

    let compressed: Vec<CompressedPoint> = commitment
        .chunks(56)
        .map(|c| CompressedPoint::from_slice(&c))
        .collect();
    ...
    let result = proof.unwrap().verify_multiple(
        &bp_gens,
        &pc_gens,
...
pub fn sum_check(
    ...
    for i in additional_input_values {
        ...
        additional_input +=
            Scalar::from_bits(i.as_slice().try_into().unwrap()) * Point(DecafPoint::GENERATOR);
    }
...
pub fn scalar_mult(lhs: Vec<u8>, rhs: Vec<u8>) -> Vec<u8> {
    if lhs.len() != 56 || rhs.len() != 56 {
        return Vec::new();
    }

    let mult = Scalar::from_bits(lhs.as_slice().try_into().unwrap())
        * Scalar::from_bits(rhs.as_slice().try_into().unwrap());
    [
        mult.as_bytes().to_vec(),
        (mult * Point(DecafPoint::GENERATOR))
            .compress()
            .as_bytes()
            .to_vec(),
    ]
...
pub fn scalar_mult_point(input_scalar: Vec<u8>, public_point: Vec<u8>) -> Vec<u8> {
...
pub fn scalar_inverse(input_scalar: Vec<u8>) -> Vec<u8> {
    ...
    [
        inv.as_bytes().to_vec(),
        (inv * Point(DecafPoint::GENERATOR))
            .compress()
            .as_bytes()
            .to_vec(),
    ]
...
pub fn scalar_mult_hash_to_scalar(input_scalar: Vec<u8>, public_point: Vec<u8>) -> Vec<u8> {
    if input_scalar.len() != 56 || public_point.len() != 56 {
        return Vec::new();
    }
...
pub fn hash_to_scalar(input: Vec<u8>) -> Vec<u8> {
...
pub fn scalar_addition(lhs: Vec<u8>, rhs: Vec<u8>) -> Vec<u8> {
    ...
    (Scalar::from_bits(lhs.as_slice().try_into().unwrap())
        + Scalar::from_bits(rhs.as_slice().try_into().unwrap()))
    .as_bytes()
    .to_vec()
}
...
pub fn scalar_to_point(scalar: Vec<u8>) -> Vec<u8> {
    ...
    (Scalar::from_bits(scalar.as_slice().try_into().unwrap()) * Point(DecafPoint::GENERATOR))
        .compress()
        .as_bytes()
        .to_vec()
}
...
pub fn point_addition(input_point: Vec<u8>, public_point: Vec<u8>) -> Vec<u8> {
    ...
    (inpoint.unwrap() + pubpoint.unwrap())
        .compress()
        .as_bytes()
        .to_vec()
}
...
pub fn sign_simple(secret: Vec<u8>, message: Vec<u8>) -> Vec<u8> {
...
pub fn verify_simple(message: Vec<u8>, signature: Vec<u8>, point: Vec<u8>) -> bool {
    ...
    let data = [message.as_slice(), pt.as_bytes(), r.as_bytes()].concat();
...
pub fn sign_hidden(x: Vec<u8>, t: Vec<u8>, a: Vec<u8>, r: Vec<u8>) -> Vec<u8> {
