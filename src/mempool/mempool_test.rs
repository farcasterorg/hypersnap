...
mod tests {
    ...
    // ---- FIP-268 LIVE_AT mempool tests ----
    //
    // Ported from upstream snapchain
    // (farcasterxyz/snapchain#899, c292bfd). The on-protocol coalescing
    // contract (LWW by `(timestamp, hash)`, per-FID/per-shard
    // single-slot, index cleanup on commit, rate-limit invalidation on
    // storage rent + storage lend) must hold for every node in the
    // network so that hypersnap stays drop-in compatible. Local
    // hyper-trie behavior is exercised separately at the engine layer
    // (see `engine_tests::*`).
